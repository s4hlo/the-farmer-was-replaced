...
def collect_pumpkin_with_drones(target_quantity):
	import no_polyculture_drone
	no_polyculture_drone.pumpkin(target_quantity)


def collect_cactus_with_drones(target_quantity):
	def drone_planter():
		...
		for _ in range(initial_world_size):
			prepare_soil_and_plant(Entities.Cactus)
			move(North)

		while get_pos_y() != initial_world_size - 1:
	...
	def drone_sorter():
		...
		if current_drone_id != 1 and get_pos_y() != initial_world_size - 1:
			move(North)
		if current_drone_id != max_drones() and get_pos_y() != initial_world_size - 1:
			spawn_drone(drone_sorter)

		while get_pos_x() != initial_world_size - 1:
	...
	if num_items(Items.Cactus) < target_quantity:
		initial_world_size = get_world_size()
		cactus_cost = get_cost(Entities.Cactus)
		navigate_to_position(0, 0)

		while num_items(Items.Cactus) < target_quantity:
			required_pumpkins = cactus_cost[Items.Pumpkin] * (initial_world_size * initial_world_size)
			if num_items(Items.Pumpkin) < required_pumpkins:
				collect_pumpkin_with_drones(required_pumpkins)
...
clear()
collect_cactus_with_drones(33554432)